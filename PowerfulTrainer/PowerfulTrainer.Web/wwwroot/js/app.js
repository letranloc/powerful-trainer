...
angular.module('config.loadingBar', []).config(["cfpLoadingBarProvider", function(cfpLoadingBarProvider) {
  cfpLoadingBarProvider.includeSpinner = true;
  return cfpLoadingBarProvider.includeBar = true;
...
angular.module("controllers.profile", []).controller("ProfileIndexCtrl", ["$scope", "$timeout", "Auth", "mdToast", function($scope, $timeout, Auth, mdToast) {
  $scope.$watch(Auth.isAuthenticated, function() {
    $scope.user = angular.copy(Auth.isAuthenticated());
    $scope.user.Birthday = new Date($scope.user.Birthday);
    return $timeout(function() {
      if ($scope.profileForm) {
        return $scope.profileForm.password.$setValidity('required', true);
      }
    });
  });
  $scope.progressing = false;
  ...
  return $scope.update = function(form) {
    if ($scope.isValidForm(form)) {
      $scope.progressing = true;
      return Auth.update($scope.user).then(function(resp) {
...
angular.module('app.directives', ['directives.classRoute', 'directives.elemReady', 'directives.onLongPress', 'directives.inputControl', 'directives.eventModel', 'directives.appScroll', 'directives.hideKeyboardOnTouchOutside', 'directives.focusMe', 'directives.fixHeight']);
...
angular.module('directives.fixHeight', []).directive('fixHeight', ["$timeout", function($timeout) {
  return {
    restrict: 'A',
    link: function(scope, element, attrs) {
      var elem, from, to;
      ...
      if (from && to) {
        return $timeout(function() {
          return to.css('height', to.height() + from.height());
        });
      }
    }
  };
...
angular.module("services.auth", []).factory("Auth", ["$http", "cookies", "AppCfg", "mdToast", function($http, cookies, AppCfg, mdToast) {
  ...
  auth = {
    ...
    update: function(user) {
      return $http.put(AppCfg.apiUrl + "/account", user, {
        headers: {
